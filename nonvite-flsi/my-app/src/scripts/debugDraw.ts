import type {
  ...
  GrowableQuadArray,
  GrowableRadiusArray,
  GrowableRandomRadiusArray,
  GrowableTriangleFanArray,
  GrowableVec2Array,
} from "./growableTypedArray";
import {
  circleCentreArray,
  circleTriangleFanArray,
  circleRadiusArray,
  particleCentreArray,
  growableColourArray,
  growableQuadArray,
...
export const debugDraw = Object.assign<Box2D.JSDraw, Partial<Box2D.JSDraw>>(
  new JSDraw(),
  {
    DrawSegment: (vert1_p: number, vert2_p: number, color_p: number): void => {
      ...
      drawBuffer.lineVertices.emplaceWithoutRealloc(
        HEAPF32[vert1_p >> 2],
        HEAPF32[(vert1_p + 4) >> 2]
      );
      drawBuffer.lineVertices.emplaceWithoutRealloc(
    ...
    DrawParticles: (
      ...
      drawBuffer.particles.centres.set(
        new Float32Array(
          HEAPF32.buffer,
          centers_p,
          count * drawBuffer.particles.centres.elemSize
        )
      );
