...
export class GrowableTypedArray<T extends TypedArray> {
  ...
  ensureFits(desiredElems: number): void {
    ...
    if (this.buffer.length <= desiredLength) {
      const newLength =
        Math.ceil(desiredLength / GrowableTypedArray.incrementSize) *
        GrowableTypedArray.incrementSize;
...
export class GrowableColourArray extends GrowableTypedArray<Float32Array> {
  ...
  emplaceWithoutRealloc(r: number, g: number, b: number, a: number): void {
  ...
  emplace(r: number, g: number, b: number, a: number): void {
...
export class GrowableRandomRadiusArray extends GrowableTypedArray<Float32Array> {
  ...
  ensureFits(desiredElems: number): void {
    ...
    if (oldLength <= desiredLength) {
      this.buffer
        .subarray(oldLength)
        .forEach((_, ix: number, arr: Float32Array): void => {
          arr[ix] = this.element();
        });
    }
  }
...
export class GrowableTriangleFanArray extends GrowableTypedArray<Float32Array> {
  ...
  emplace(x: number, y: number, radius: number): void {
    ...
    for (let i = 0; i < this.vertices; i++) {
      ...
      this.buffer[this.length * this.elemSize + 2 + 2 * i] =
        x + Math.cos(angle) * radius;
      this.buffer[this.length * this.elemSize + 2 + 2 * i + 1] =
        y + Math.sin(angle) * radius;
    }
    this.buffer[this.length * this.elemSize + 2 + 2 * this.vertices] =
      this.buffer[this.length * this.elemSize + 2];
    this.buffer[this.length * this.elemSize + 2 + 2 * this.vertices + 1] =
      this.buffer[this.length * this.elemSize + 3];
    this.length = newLength;
  }
}
